...
class TestIndexWorkerProcessOne:
    ...
    async def test_process_text_task_success(
        ...
        with (
            patch(_PATCH_STORE, return_value=mock_store),
    ...
    async def test_process_task_failure(
        ...
        with (
            ...
            mock_builder = MagicMock()
            mock_builder.build_from_pdfs = AsyncMock(side_effect=RuntimeError("PDF parsing failed"))
            mock_builder_cls.return_value = mock_builder
    ...
    async def test_unknown_file_type_raises(
        ...
        has_failed = any(c.args[0] == "test-004" and "failed" in str(c.args) for c in all_calls)
...
class TestIndexWorkerStart:
    ...
    async def test_start_once_mode(self) -> None:
        ...
        with (
            patch("retrieval.pipelines.index_worker.init_redis", new_callable=AsyncMock),
            ...
            patch.object(
                worker, "_process_one", new_callable=AsyncMock, return_value=True
            ) as mock_process,
        ):
