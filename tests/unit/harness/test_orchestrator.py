...
class TestHarnessOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_execute_pre_chain_all_pass(self) -> None:
        orc = HarnessOrchestrator()
        pre = PreExecContext(
            node_name="writer",
            raw_input="帮我写一份行业报告",
        ...
        results = await orc.execute_pre(pre)
        assert len(results) > 0
        assert all(r.decision != HandlerDecision.REJECT for r in results)

    @pytest.mark.asyncio
    async def test_execute_post_chain_all_pass(self) -> None:
        orc = HarnessOrchestrator()
        post = PostExecContext(
            node_name="writer",
            raw_output="# 报告\n\n风险提示\n\n内容。",
        )
        results = await orc.execute_post(post)
        assert len(results) > 0

    @pytest.mark.asyncio
    async def test_execute_pre_short_circuits_on_reject(self) -> None:
